...
def get_optimizer_scheduler(model_name,
    ...
    ## cautious sgd variance
    # csgd_optimizer = utils.csgd.CSGD(net.parameters(), lr=lr, momentum=momentum, weight_decay=weight_decay)
    # sam_csgd = utils.sam.SAM(net.parameters(), utils.csgd.CSGD, lr=lr, weight_decay=weight_decay, momentum=momentum)
    # fsam_csgd = utils.fsam.FriendlySAM(net.parameters(), utils.csgd.CSGD, 
    #                             rho=args.rho, sigma=args.sigma, lmbda=args.lmbda, adaptive=0, 
    #                             lr=lr, momentum=momentum, weight_decay=weight_decay,
    #                             nesterov=False)    
    ...
    if model_name in ["convmixer", "vit_cifar", "deit_base_patch16_224","dinov3_l16" ] : # "FastViT-SA24"
        ...
        if optim_name in ['sam', 'fmfp'] : 
            ...
            optimizer = fsam_sgd
        # elif optim_name == 'csgd':
        #     optimizer = csgd_optimizer
        # elif optim_name == 'csam':
        #     optimizer = sam_csgd
        # elif optim_name == 'cfsam':
        #     optimizer = fsam_csgd
        else :
