# adaptive cautions-SGD (must be momentum based SGD, therefore not mentioned in the original paper)
''' # in newer torch
from typing import List, Optional
import torch
from torch import Tensor
from torch.utils._foreach_utils import _get_fused_kernels_supported_devices
from torch.optim.optimizer import Optimizer, _use_grad_for_differentiable, _default_to_fused_or_foreach, DeviceDict

# these code are majorly from the implementation of SGD in pytorch
...
def _single_tensor_sgd(
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        # we have to apply cautions in a momentum based method, so assert here
        assert momentum != 0, 'cautions only works in a momentum based optimizer'
        
        if momentum != 0:
            buf = momentum_buffer_list[i]

            # modify here with a cautions optimizer approach
        ...
        param.add_(grad * mask / (mask.mean() + 1e-6), alpha=-lr)
...
def _single_tensor_sgd(params: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        param.add_(d_p * mask / (mask.mean() + 1e-6), alpha=-lr)
