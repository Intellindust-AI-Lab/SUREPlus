...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, register_hook=False):
        ...
        rope_sincos = self.rope_embed(H=patch_grid_h, W=patch_grid_w)
    ...
    def forward(self, x, register_hook=False):
        """
        Forward function for training/inference.
        Returns only the final layer CLS token.
        """
        B, C, H, W = x.shape
        ...
        # Compute RoPE positional embedding
        patch_grid_h = H // self.patch_size
        patch_grid_w = W // self.patch_size
        rope_sincos = self.rope_embed(H=patch_grid_h, W=patch_grid_w)
        ...
        # Return final CLS token only
        cls_token_final = x[:, 0]   # shape: [B, D]
...
def make_vit_tiny():
    ...
    backbone = VisionTransformer(**configs)
    head = torch.nn.Linear(192, 1000).cuda()
    net = nn.Sequential(
            backbone,
            head
        ) 
