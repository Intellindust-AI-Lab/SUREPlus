...
class ResNet18_32x32_csc(nn.Module):
    def __init__(self, block=BasicBlock, num_blocks=None, num_classes=10):
        super(ResNet18_32x32_csc, self).__init__()
        ...
        self.classifier = Classifier(512 * block.expansion, num_classes, 8.0)
    ...
    def get_fc(self):
        fc = self.classifier
        weight = fc.weight.t().detach().cpu().numpy()
        bias = fc.bias.detach().cpu().numpy() if fc.bias is not None else None
        return weight, bias
